...
export const EpubReaderV2 = forwardRef<EpubReaderV2Handle, EpubReaderV2Props>(
  function EpubReaderV2(props, ref) {
    ...
    const [spinePageCounts, setSpinePageCounts] = useState<
      Array<number | null>
    >([])
    ...
    panelsOpenRef.current = tocOpen || settingsOpen || customizeOpen
    ...
    const goToHrefRef = useRef<
      (href: string, behavior?: ScrollBehavior) => void
    >(() => {})
    ...
    const pageViewport = useMemo(() => {
      const availableWidth = Math.max(
        0,
        containerWidth - marginPx.horizontal * 2,
      )
      const availableHeight = Math.max(
        0,
        containerHeight - marginPx.vertical * 2,
      )
    ...
    const hudVisible = hudActive || tocOpen || settingsOpen || customizeOpen
    ...
    const updateSettings = useCallback(
      (next: Partial<EpubReaderV2Settings>) => {
    ...
    const loadSpine = useCallback(
      async (
        ...
        pendingNavRef.current = options ?? null
        try {
    ...
    const setScrollLocation = useCallback(
      (next: { spineIndex: number; scrollTop: number; href: string }) => {
        locationRef.current = {
          spineIndex: next.spineIndex,
          pageIndex: 0,
    ...
    const goToHref = useCallback(
      (href: string, behavior?: ScrollBehavior) => {
        ...
        const currentSpinePath =
          publication.spine[spineIndex]?.href ??
    ...
    const goToPage = useCallback(
      ...
      [publication, spineIndex, setLocation],
    )
      ...
      try {
        ...
        const releaseAfterScrollSettles = () => {
          ...
          const waitForScroll = () => {
            ...
            const expected =
              Math.max(0, locationRef.current.pageIndex) *
        ...
        if (pending?.fragment) {
          const ok = goToAnchorInCurrentDoc(pending.fragment, pending.behavior)
          if (ok) {
            if (pending.consumeNavigationId) {
              onConsumePendingNavigationRef.current?.(
                pending.consumeNavigationId,
              )
            }
        ...
        if (
          typeof pending?.textOffset === 'number' &&
          Number.isFinite(pending.textOffset)
        ) {
        ...
        if (settings.flowMode === 'scrolled') {
          const desiredScrollTop =
            pending?.scrollTop ?? locationRef.current.scrollTop ?? 0
          layout.viewportEl.scrollTo({
            left: 0,
            top: Math.max(0, desiredScrollTop),
            behavior: pending?.behavior ?? 'auto',
          })
          setScrollLocation({
            spineIndex,
            scrollTop: layout.viewportEl.scrollTop,
          ...
          if (pending?.consumeNavigationId) {
            onConsumePendingNavigationRef.current?.(pending.consumeNavigationId)
          }
          pendingNavRef.current = null
          releaseAfterScrollSettles()
          return
        ...
        const desiredPageIndex =
          pending?.pageIndex ?? locationRef.current.pageIndex
        if (desiredPageIndex > 0 && layout.totalPages <= 1) {
    ...
    useEffect(() => {
      ...
      const persist = () => {
        ...
        const persistenceId = persistenceIdRef.current
        const layout = appliedLayoutRef.current
        if (!persistenceId || !layout || layout.viewportEl !== viewportEl)
          return
        const top = viewportEl.scrollTop
        if (Math.abs(top - last) < 1) return
        last = top
        locationRef.current = { spineIndex, pageIndex: 0, scrollTop: top }
        storeLocation(persistenceId, {
          spineIndex,
          pageIndex: 0,
          scrollTop: top,
    ...
    useEffect(() => {
      ...
      const persist = () => {
        ...
        locationRef.current = { spineIndex, pageIndex: page, scrollTop: 0 }
        setPageIndex(page)
        storeLocation(persistenceId, {
          spineIndex,
          pageIndex: page,
    ...
    useLayoutEffect(() => {
      ...
      const publishSelection = () => {
        ...
        const bookId = storageId ?? publicationRef.current?.bookId ?? ''
        const bookTitle =
    ...
    useEffect(() => {
      ...
      ;(async () => {
        try {
          ...
          const pub = await loadEpubPublication(zip, bookUrl)
          if (!alive) return
          setPublication(pub)
          ...
          const persistenceId = storageId ?? pub.bookId
          persistenceIdRef.current = persistenceId
          const stored = loadStoredLocation(persistenceId)
    ...
    }, [bookUrl, storageId, authHeadersKey, reloadToken, loadSpine])

    useEffect(() => {
      if (!pendingNavigation) return
      const debugEnabled =
        typeof window !== 'undefined' &&
        (((window as any).__MFV2_DEBUG_CHAT_MENTIONS as any) ||
          ((window as any).__MFV2_DEBUG_READER_NAV as any) ||
      ...
      const debug = (...args: Array<unknown>) => {
        ...
        console.log('[mfv2][reader][nav]', ...args)
      }

      const bookId = storageId ?? ''
      debug('pending', {
        pendingNavigation,
        storageId: bookId,
      ...
      if (
        ...
        pendingNavigation.bookId !== bookId
      ) {
        debug('skip:mismatchedBook', {
          pendingBookId: pendingNavigation.bookId,
          storageId: bookId,
        })
        return
      }
      ...
      if (pendingNavigation.href) {
        debug('href', pendingNavigation.href)
        if (!publication) {
          debug('href:blocked:noPublication')
          return
        }
      ...
      debug('goToTextOffset', { textOffset, ok })
      if (ok) {
        onConsumePendingNavigationRef.current?.(pendingNavigation.id)
        return
      }

      debug('defer:textOffset', { textOffset })
    ...
    useEffect(() => {
      ...
      const bookId = storageId ?? pub.bookId ?? ''
      const bookTitle = (pub.title ?? '').trim() || 'Book'
      ...
      cb({ bookId, bookTitle, chapters })
    }, [publication, storageId, tocItemsFlat])
        ...
        >
          <div className="mfv2-reader__topbar">
            ...
            <div className="mfv2-reader__topbarRight">
              <button
                className={`mfv2-reader__iconBtn ${settingsOpen ? 'is-active' : ''}`}
          ...
          {(tocOpen || settingsOpen) && !customizeOpen && (
            <div
              ...
              <div style={{ display: 'grid', gap: 2 }}>
                {tocItemsFlat.length === 0 ? (
                  ...
                  tocItemsFlat.map(({ item, depth }) => (
                    <button
                      key={`${item.href}:${depth}`}
                      className={`mfv2-reader__tocItem ${normalizePath(splitHref(item.href).path) === normalizePath(currentHref) ? 'is-active' : ''}`}
                      style={{ paddingLeft: 12 + depth * 16 }}
                      type="button"
                      onClick={() => {
                        showHud()
                        goToHref(item.href, 'auto')
                        setTocOpen(false)
                      }}
                      title={item.title}
                    >
        ...
        {customizeOpen && (
          <div className="mfv2-reader__customizePanel">
            <div className="mfv2-reader__customizeHeader">
              ...
              <span className="mfv2-reader__customizeTitle">
                Customize Theme
              </span>
              <button
                type="button"
