...
function themeVars(themePreset: EpubReaderV2ThemePreset): { bg: string; fg: string; link: string } {
...
export function applyReaderLayout(options: {
  ...
  root.style.setProperty('--mfv2-text-align', settings.textAlign === 'auto' ? 'initial' : settings.textAlign);

  if (settings.fontFamily === 'serif') {
    root.style.setProperty('--mfv2-font-family', `Iowan Old Style, Georgia, "Times New Roman", Times, serif`);
  } else if (settings.fontFamily === 'sans') {
    root.style.setProperty('--mfv2-font-family', `-apple-system, BlinkMacSystemFont, "Segoe UI", Inter, system-ui, sans-serif`);
  } else {
...
export function scrollToPage(options: { layout: AppliedLayout; pageIndex: number; behavior?: ScrollBehavior }) {
...
export function findPageIndexForElement(options: { layout: AppliedLayout; element: Element }): number {
  const { layout, element } = options;
  const rect = element.getBoundingClientRect();
  const viewportRect = layout.viewportEl.getBoundingClientRect();
  const absoluteLeft = rect.left - viewportRect.left + layout.viewportEl.scrollLeft;
  return Math.max(0, Math.min(layout.totalPages - 1, Math.floor(absoluteLeft / layout.pageWidth)));
}
